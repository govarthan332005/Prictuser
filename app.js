// ============================================================================
// ROULETTE USER APP — v3.2 PERFORMANCE EDITION
// ✓ localStorage cache for training data (instant load, no waiting)
// ✓ Debounced + throttled Firebase listeners
// ✓ Lazy tab rendering (only render visible tab)
// ✓ Memoized engine results
// ✓ requestAnimationFrame for DOM updates
// ✓ Event delegation (no per-button listener growth)
// ✓ limitToLast for bounded queries
// ============================================================================
import {
  db, auth, ref, set, push, get, onValue, update, query, orderByKey, limitToLast,
...
// Hard cap on training data we keep client-side to keep things fast
const TRAINING_CAP = 2000;
const CACHE_KEY = 'rp_train_cache_v2';
...
// debounce: collapse rapid bursts into a single trailing call
...
// rAF throttle for smoother DOM repaints
...
const cache = {
  loadTraining() {
    try {
      ...
      if (obj.v !== CACHE_VERSION || !Array.isArray(obj.data)) return null;
      return obj;
    } catch { return null; }
  },
  saveTraining(data) {
    try {
      ...
      const trimmed = data.slice(-TRAINING_CAP);
      localStorage.setItem(CACHE_KEY, JSON.stringify({
        v: CACHE_VERSION, data: trimmed, ts: Date.now(), total: data.length
      }));
    } catch (e) {
      // quota? clear and retry once with fewer items
      try {
        localStorage.removeItem(CACHE_KEY);
        const tiny = data.slice(-500);
        localStorage.setItem(CACHE_KEY, JSON.stringify({ v: CACHE_VERSION, data: tiny, ts: Date.now(), total: data.length }));
      } catch {}
    }
  },
  loadHistory() {
    try {
      const raw = localStorage.getItem('rp_user_hist_v2');
      if (!raw) return null;
  ...
  saveHistory(arr) {
    try {
      localStorage.setItem('rp_user_hist_v2', JSON.stringify(arr.slice(0, 200)));
    } catch {}
  }
};
...
let trainingData = [];
let trainingTotal = 0; // total in DB (may exceed local cap)
let predictSeq  = [];
...
let _userHistUnsub = null;
let activeTab = 'predict';

// Prime engine instantly from cache so login → app feels snappy
(function bootstrapFromCache() {
  const c = cache.loadTraining();
  if (c) {
    trainingData = c.data || [];
    trainingTotal = c.total || trainingData.length;
    engine.setHistory(trainingData);
  }
  const h = cache.loadHistory();
  if (h && Array.isArray(h)) userPredHistory = h;
})();
...
$('signOutBtn').addEventListener('click', () => {
  if (currentUser?.uid === 'guest') { currentUser = null; location.reload(); }
  else signOut(auth);
});
...
function enterApp() {
  ...
  initApp();
  startBreakTimer();
  if (!settings.tourSeen) setTimeout(() => $('welcomeTour').classList.remove('hidden'), 350);
...
$$('.tab').forEach(t => t.addEventListener('click', () => {
  ...
  // Lazy: only render when needed
  if (activeTab === 'insights') renderInsights();
  if (activeTab === 'history') renderHistory();
  // Avoid smooth scroll (laggy on weak phones); use instant
  window.scrollTo(0, 0);
}));
...
$$('.method-tab').forEach(b => b.addEventListener('click', () => {
  ...
  document.querySelector(`.input-method[data-method-panel="${predictMethod}"]`)?.classList.add('active');
  vibrate(6);
}));
...
$$('.mode-tab').forEach(b => b.addEventListener('click', () => {
  ...
  b.classList.add('active');
  if (isLive) { liveMode = b.dataset.livemode; updateLivePrediction(); }
  else if (b.dataset.pmode) { predictMode = b.dataset.pmode; persistSequence(); }
}));

// ---------- DATA LOADING (lean, debounced, cached) ----------
const refreshDataDependent = debounce(() => {
  updateDataPill();
  if (activeTab === 'insights') renderInsights();
}, 150);

function initApp() {
  // Update pill IMMEDIATELY from cache (no waiting on Firebase)
  updateDataPill();

  if (_historyUnsub) try { _historyUnsub(); } catch {}

  // Use limitToLast to bound payload — only fetch most recent N entries
  const histQuery = query(ref(db, 'history'), orderByKey(), limitToLast(TRAINING_CAP));

  _historyUnsub = onValue(histQuery, snap => {
    const hist = snap.val() || {};
    // Sort by timestamp; missing timestamps treated as 0
    const arr = Object.values(hist).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
    trainingData = arr;
    trainingTotal = arr.length; // approximate; we don't fetch the whole tree
    engine.setHistory(trainingData);
    cache.saveTraining(trainingData);
    refreshDataDependent();
  }, err => {
...
function buildNumberPads() {
  const buildPad = (id, onTap) => {
    ...
    c.innerHTML = html;
    // Single delegated listener (no per-button growth)
    c.addEventListener('click', e => {
      const b = e.target.closest('.num-btn');
      if (!b) return;
      const n = parseInt(b.dataset.n);
      // Lightweight visual feedback via class (CSS handles transition, not inline style)
...
$('undoBtn').addEventListener('click', () => {
  predictSeq.pop(); scheduleRenderSeq(); persistSequence(); vibrate(8);
});
$('clearBtn').addEventListener('click', () => {
  if (predictSeq.length === 0) return;
  predictSeq = [];
  scheduleRenderSeq(); persistSequence();
  $('predictionCard').classList.add('hidden');
  vibrate(12);
});

const scheduleRenderSeq = rafThrottle(renderSeq);
function renderSeq() {
  const display = $('seqDisplay');
  if (!display) return;
  const html = predictSeq.map((e, i) => {
    let label, cls;
    if (typeof e.n === 'number') { label = e.n; cls = e.color || ''; }
...
// ---------- AUTO-SAVE ----------
const persistSequence = debounce(() => {
...
function restoreSequence() {
  try {
    ...
    if (Date.now() - data.ts > 86400000) { localStorage.removeItem('rp_seq'); return; }
    predictSeq = Array.isArray(data.seq) ? data.seq : [];
...
$('demoBtn').addEventListener('click', () => {
  ...
  $$('.input-method').forEach(x => x.classList.toggle('active', x.dataset.methodPanel === 'numbers'));
  renderSeq(); persistSequence();
...
$('predictBtn').addEventListener('click', () => {
  ...
  try {
    if (predictMode === 'even_odd') {
      if (eoArr.length < 5) return toast(`Need 5+ Even/Odd entries (have ${eoArr.length})`, 'error');
      result = engine.predict(eoArr, 'even_odd');
    } else if (predictMode === 'color') {
      if (colArr.length < 5) return toast(`Need 5+ Red/Black entries (have ${colArr.length})`, 'error');
      ...
      if (!eo && !col) return toast('Need 5+ of either type', 'error');
      result = { eo, col, both: true };
    }
  } catch (e) { return toast('Prediction error: ' + e.message, 'error'); }
  ...
  if (currentUser && currentUser.uid !== 'guest') {
    (async () => {
      try {
        ...
        const sanitized = deepClean({ ...lastPrediction, status: 'pending' });
...
function showPrediction(result) {
  ...
  if (result.both) {
    ...
    if (r2) parts.push(`<div style="margin-top:14px"><div class="pred-value" style="font-size:52px">${r2.prediction || '?'}</div><div class="pred-name">${labelOf(r2.prediction)}</div></div>`);
  ...
  $('predMeta').innerHTML = `
    Trained on <b>${trainingData.length.toLocaleString()}</b> spins ·
    <b>${predictSeq.length}</b> your inputs ·
    <b>${agreement}%</b> model agreement<br>
    <span style="opacity:0.7">Markov · Pattern · Streak · Bayesian · Cyclic · Recency · Bias · Sector · Neural</span>
  `;
  // Use rAF for smoother scroll-into-view
...
function checkConfWarn(conf) {
  if (conf < settings.confThreshold) {
    ...
    $('confWarn').textContent = `⚠ Low confidence (${Math.round(conf)}%) — model isn't sure. Add more data.`;
  } else {
...
function renderProbs(probs, winner) {
  if (!probs) { $('probGrid').innerHTML = ''; return; }
...
async function recordFeedback(correct) {
  ...
  beep(correct ? 1200 : 400, 100);

  if (currentUser && currentUser.uid !== 'guest' && lastPrediction.id) {
    // Fire-and-forget for snappy UI
    (async () => {
      try {
        ...
        await update(userRef, {
          predictionCount: total, correctCount,
          accuracy: correctCount / total * 100, lastActive: Date.now()
        });
      } catch (e) { console.warn('Feedback save failed', e); }
    })();
  } else if (currentUser?.uid === 'guest') {
    userPredHistory.unshift({ ...lastPrediction, status, id: 'g_' + Date.now() });
    cache.saveHistory(userPredHistory);
    if (activeTab === 'history') renderHistory();
  }
  $('predictionCard').classList.add('hidden');
  predictSeq = [];
...
async function loadUserHistory() {
  ...
  if (_userHistUnsub) try { _userHistUnsub(); } catch {}
  // Bound history fetch
  const histQuery = query(ref(db, `userPredictions/${currentUser.uid}`), orderByKey(), limitToLast(200));
  _userHistUnsub = onValue(histQuery, snap => {
    const data = snap.val() || {};
...
// ---------- LIVE TRACK ----------
const updateLiveScheduled = rafThrottle(_updateLivePrediction);
function addToLive(entry) {
  ...
  if (lastLivePrediction && v && lastLivePrediction.prediction) {
    ...
    if (v === lastLivePrediction.prediction) {
      ...
      beep(1200, 70);
    } else { liveStats.streak = 0; beep(400, 70); }
  }
  liveSeq.push(cleaned);
  if (liveSeq.length > 200) liveSeq.shift();
  scheduleRenderLiveSeq();
  updateLiveScheduled();
}
$('liveUndoBtn').addEventListener('click', () => {
  if (liveSeq.length === 0) return toast('Nothing to undo', 'warn');
  liveSeq.pop();
  scheduleRenderLiveSeq(); updateLiveScheduled();
  vibrate(8);
});
$('liveClearBtn').addEventListener('click', () => {
  if (liveSeq.length === 0 && liveStats.total === 0) return;
  if (!confirm('Reset live tracking session?')) return;
  liveSeq = [];
  liveStats = { correct: 0, total: 0, streak: 0, maxStreak: 0 };
  lastLivePrediction = null;
  renderLiveSeq(); _updateLivePrediction();
  toast('Live session reset', 'success');
});
const scheduleRenderLiveSeq = rafThrottle(renderLiveSeq);
...
function updateLivePrediction() { updateLiveScheduled(); }
function _updateLivePrediction() {
  ...
  if (arr.length < 5) {
    ...
    try {
      ...
      $('liveConf').textContent = `${labelOf(result.prediction)} · ${result.confidence}% · ${result.agreement}% agree`;
      lastLivePrediction = result;
    } catch { $('liveNext').textContent = '—'; $('liveConf').textContent = 'Prediction error'; lastLivePrediction = null; }
  }
...
function renderHistory() {
  ...
  const accNum = settled.length > 0 ? (correct / settled.length * 100) : 0;
  const acc = settled.length > 0 ? accNum.toFixed(1) : '0';
  ...
  const ring = $('accRing');
  if (ring) ring.style.strokeDashoffset = 283 - (accNum / 100 * 283);
  ...
  if (accNum >= 60 && settled.length >= 10) badges.push({ icon: '🏆', label: 'Sharp', gold: true });
  let maxRun = 0, run = 0;
  ...
  // Render only first 50 to keep DOM light
  $('historyList').innerHTML = userPredHistory.slice(0, 50).map(p => {
    ...
    return `
      <div class="history-item ${p.status || 'pending'}">
        ...
        <div class="history-status-icon">${p.status === 'correct' ? '✅' : p.status === 'wrong' ? '❌' : '⏳'}</div>
      </div>`;
  }).join('');
}
$('exportHistBtn').addEventListener('click', () => {
  ...
  userPredHistory.forEach(p => {
    ...
    rows.push([ts, p.mode || '', pred, conf, p.status || 'pending', `"${seq}"`].join(','));
  });
...
$('clearHistBtn').addEventListener('click', () => {
  if (userPredHistory.length === 0) return toast('Nothing to clear', 'warn');
...
// ---------- INSIGHTS (lazy + memoized) ----------
let _insightsKey = '';
function renderInsights() {
  const total = trainingData.length;
  // Skip recompute if unchanged
  const key = total + ':' + (trainingData[trainingData.length - 1]?.timestamp || 0);
  if (key === _insightsKey) return;
  _insightsKey = key;
  ...
  if (total === 0) {
    ...
    $('insEvenBias').textContent = '—'; $('insRedBias').textContent = '—'; $('insPattern').textContent = '—';
    return;
  }
  ...
  } else { $('insEvenBias').textContent = '—'; $('insEvenSub').textContent = 'No E/O data'; }
  if (cols.length > 0) {
    $('insRedBias').textContent = (reds / cols.length * 100).toFixed(1) + '%';
    $('insRedSub').textContent = `${reds} red / ${cols.length - reds} black`;
  } else { $('insRedBias').textContent = '—'; $('insRedSub').textContent = 'No R/B data'; }

  const patterns = {};
  for (let i = 0; i < eos.length - 3; i++) {
    const k = eos[i].even_odd + eos[i+1].even_odd + eos[i+2].even_odd;
    patterns[k] = (patterns[k] || 0) + 1;
  }
  const sorted = Object.entries(patterns).sort((a, b) => b[1] - a[1]);
  if (sorted.length) { $('insPattern').textContent = sorted[0][0]; $('insPatternSub').textContent = `${sorted[0][1]} occurrences`; }
  else { $('insPattern').textContent = '—'; $('insPatternSub').textContent = 'Need more data'; }

  // Pulse canvas (high-DPI, single draw)
  ...
  if (canvas) {
    ...
    if (recent.length > 0) {
      const cellW = cssW / recent.length;
      // Use solid colors instead of per-cell gradient for huge perf win
      recent.forEach((d, i) => {
        ctx.fillStyle = d.even_odd === 'E' ? '#8b5cf6' : '#fbbf24';
        ctx.fillRect(i * cellW, 0, cellW + 0.5, cssH);
      });
    }
  }
  ...
  $('transGrid').innerHTML = `
    <div class="trans-cell"><span>E → E</span><b>${fmt(trans.EE, trans.EE + trans.EO)}</b></div>
    <div class="trans-cell"><span>E → O</span><b>${fmt(trans.EO, trans.EE + trans.EO)}</b></div>
    <div class="trans-cell"><span>O → E</span><b>${fmt(trans.OE, trans.OE + trans.OO)}</b></div>
    <div class="trans-cell"><span>O → O</span><b>${fmt(trans.OO, trans.OE + trans.OO)}</b></div>
  `;
}
...
function setTourStep(n) {
  ...
  $('tourNext').textContent = tourStep === totalSteps ? "Let's go! 🚀" : 'Next →';
}
$('tourNext').addEventListener('click', () => {
  if (tourStep === totalSteps) {
    $('welcomeTour').classList.add('hidden');
    settings.tourSeen = true; saveSetting('tour', 'done'); return;
  }
  setTourStep(tourStep + 1);
});
$('tourSkip').addEventListener('click', () => {
  $('welcomeTour').classList.add('hidden');
  settings.tourSeen = true; saveSetting('tour', 'done');
});
$$('.tour-dots .dot').forEach((d, i) => {
  d.addEventListener('click', () => setTourStep(i + 1));
...
$('autoSaveToggle').addEventListener('change', e => {
  settings.autoSave = e.target.checked; saveSetting('autosave', settings.autoSave);
...
function restartBreakTimer() { clearTimeout(breakTimer); if (settings.breakReminder) startBreakTimer(); }
$('breakClose').addEventListener('click', () => {
  $('breakModal').classList.add('hidden');
  sessionStart = Date.now(); startBreakTimer();
});
...
document.addEventListener('keydown', e => {
  ...
  const onPredict = activeTab === 'predict';
  const onTrack = activeTab === 'track';
  ...
  if (e.key === 'Backspace') {
    if (onPredict) { predictSeq.pop(); scheduleRenderSeq(); persistSequence(); }
    if (onTrack)   { liveSeq.pop(); scheduleRenderLiveSeq(); updateLiveScheduled(); }
  }
  if (e.key === 'Enter' && onPredict) $('predictBtn').click();
});

// Disable iOS pinch-zoom
