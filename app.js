// ============================================================================
// ROULETTE USER APP — v2.0
// Self-contained AI prediction app for end users
// ============================================================================
import {
...
// ---------- App state ----------
let engine = new PredictionEngine();
let trainingData = [];
let predictSeq  = []; // chronological raw entries; each may carry {n, eo, color}
let predictMode = 'even_odd';
let predictMethod = 'numbers'; // 'numbers' | 'manual'
let liveSeq  = [];
...
$('signupBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    await set(ref(db, `users/${cred.user.uid}`), {
      email, createdAt: Date.now(), predictionCount: 0, correctCount: 0, accuracy: 0
    });
...
$$('.mode-tab').forEach(b => b.addEventListener('click', () => {
  ...
  const group = isLive ? 'livemode' : 'pmode';
...
$('demoBtn').addEventListener('click', () => {
  ...
  // Realistic-looking sample (mix of E/O/R/B with some patterns)
  ...
  renderSeq();
  persistSequence();
  toast('🎬 Demo data loaded — now hit PREDICT', 'success');
});
...
$('predictBtn').addEventListener('click', async () => {
  ...
  // Convert to engine-friendly arrays
  ...
  if (predictMode === 'even_odd') {
    ...
    // 'both' — predict each independently when there's enough data
  ...
  if (currentUser && currentUser.uid !== 'guest') {
    try {
      ...
      await set(hRef, { ...lastPrediction, status: 'pending' });
...
function showPrediction(result) {
  ...
  if (result.both) {
    ...
    if (r2) parts.push(`<div style="margin-top:10px"><div class="pred-value" style="font-size:42px">${r2.prediction || '?'}</div><div class="pred-name">${labelOf(r2.prediction)}</div></div>`);
  ...
  $('predMeta').innerHTML = `
    Based on <b>${trainingData.length}</b> training spins +
    <b>${predictSeq.length}</b> your inputs<br>
    Engine: Markov · Pattern · Streak · Bayesian · Cyclic
  `;
...
function checkConfWarn(conf) {
  if (conf < settings.confThreshold) {
    ...
    $('confWarn').textContent = `⚠ Low confidence (${Math.round(conf)}%) — model isn't sure. Consider waiting for more data.`;
  } else {
...
function renderModelBars(models) {
  ...
  const names = { markov: 'Markov Chain', pattern: 'Pattern Match', streak: 'Streak Analysis', bayesian: 'Bayesian', cyclic: 'Cyclic Detect' };
...
function addToLive(entry) {
  // First validate against last prediction
  const v = liveMode === 'even_odd' ? entry.eo : entry.color;
...
function updateLivePrediction() {
  ...
  if (arr.length < 5) {
    ...
    $('liveConf').textContent = `${labelOf(result.prediction)} · ${result.confidence}% confidence`;
...
function renderHistory() {
  ...
  if (total === 0) {
    $('historyList').innerHTML = '<div style="text-align:center;color:var(--text-dim);padding:30px;font-size:13px">No predictions yet. Make your first one!</div>';
    return;
  }
  ...
  $('historyList').innerHTML = userPredHistory.slice(0, 50).map(p => {
    ...
    return `
      <div class="history-item ${p.status}">
        ...
        <div style="font-size:18px">${p.status === 'correct' ? '✓' : p.status === 'wrong' ? '✕' : '⏳'}</div>
      </div>
    `;
  }).join('');
}
...
function renderInsights() {
  ...
  recent.forEach((d, i) => {
    ...
    if (d.even_odd === 'E') {
      grad.addColorStop(0, '#4a90e2'); grad.addColorStop(1, '#2a6dc2');
    } else {
      grad.addColorStop(0, '#e25c4a'); grad.addColorStop(1, '#c2382a');
    }
    ctx.fillStyle = grad;
...
document.addEventListener('keydown', e => {
  ...
  // Only process keys on Predict tab when prediction card not focused
  ...
  // Numbers 0-9: build to a roulette number (one digit only for now)
  if (/^[0-9]$/.test(k)) {
    // For two-digit, need to track timing — for simplicity let's tap number directly if it's <=36
    const n = parseInt(k);
    if (n >= 0 && n <= 9) {
