// ============================================================================
// ROULETTE USER APP — v3.1 STUDENT EDITION
// 9-model ensemble · calibrated confidence · adaptive weights · ALL BUGS FIXED
// ============================================================================
import {
...
// Deep-clean nested objects (for Firebase write safety)
...
// ---------- METHOD TABS (FIXED: precise selector for input methods) ----------
$$('.method-tab').forEach(b => b.addEventListener('click', () => {
  ...
  // Use NEW data-method-panel attribute so it doesn't collide with anything else
  $$('.input-method').forEach(x => x.classList.remove('active'));
  const panel = document.querySelector(`.input-method[data-method-panel="${predictMethod}"]`);
  if (panel) panel.classList.add('active');
  vibrate(8);
}));

// ---------- MODE TABS (FIXED: scoped by attribute) ----------
$$('.mode-tab').forEach(b => b.addEventListener('click', () => {
  const isLive = b.dataset.livemode !== undefined;
  // Only update siblings within the same parent
  const siblings = b.parentElement.querySelectorAll('.mode-tab');
  siblings.forEach(x => x.classList.remove('active'));
  b.classList.add('active');
  if (isLive) {
    liveMode = b.dataset.livemode;
    updateLivePrediction();
  } else if (b.dataset.pmode) {
...
function initApp() {
  ...
  _historyUnsub = onValue(ref(db, 'history'), snap => {
    ...
    updateDataPill();
    const insightsVisible = document.querySelector('.tab-panel[data-panel="insights"]')?.classList.contains('active');
...
// FIXED: Manual E/O / R/B buttons — using new precise attribute data-manual
...
async function recordFeedback(correct) {
  ...
  if (currentUser && currentUser.uid !== 'guest' && lastPrediction.id) {
    ...
    // For guests, store in local history only
...
async function loadUserHistory() {
  ...
  _userHistUnsub = onValue(ref(db, `userPredictions/${currentUser.uid}`), snap => {
    const data = snap.val() || {};
...
function updateLivePrediction() {
  ...
  if (arr.length < 5) {
    ...
    try {
      ...
      $('liveNext').textContent = '—';
      $('liveConf').textContent = 'Prediction error';
      lastLivePrediction = null;
    }
  }
...
function renderInsights() {
  ...
  if (total === 0) {
    ...
    $('insEvenBias').textContent = '—';
    $('insRedBias').textContent = '—';
    $('insPattern').textContent = '—';
    return;
  }
  ...
  if (eos.length > 0) {
    ...
    $('insEvenBias').textContent = '—';
    $('insEvenSub').textContent = 'No E/O data';
  }
  if (cols.length > 0) {
    ...
    $('insRedBias').textContent = '—';
    $('insRedSub').textContent = 'No R/B data';
  }
  ...
  if (sorted.length) {
    ...
    $('insPattern').textContent = '—';
    $('insPatternSub').textContent = 'Need more data';
  }

  // Pulse canvas with high-DPI support
  ...
  // Transitions (FIXED: handle empty cases properly)
...
// ---------- KEYBOARD SHORTCUTS (FIXED: 0 works now) ----------
document.addEventListener('keydown', e => {
  ...
  const onPredict = document.querySelector('.tab-panel[data-panel="predict"]')?.classList.contains('active');
  const onTrack = document.querySelector('.tab-panel[data-panel="track"]')?.classList.contains('active');
